...
\begin{figure}[H]
    \centering
    \includegraphics[angle=0,width=\textwidth]{../resources/HotelDB_ER_Finale.jpeg}
...
\begin{figure}[H]
    \centering
    \includegraphics[angle=0,width=0.75\textwidth]{../resources/ER/HotelDB_ER_GeneralizzazionePersona.png}
...
\newline
Non esiste, nello schema, un attributo \textbf{StatoAttuale} direttamente su \textbf{PRENOTAZIONE}: lo stato corrente di una prenotazione è un'informazione \textit{derivabile} dallo storico delle variazioni, corrispondendo alla riga con \textbf{DataOraCambio} più recente per quella prenotazione. Introdurre un attributo separato per lo stato corrente duplicherebbe un'informazione già ricavabile dallo storico, violando lo stesso criterio di \textbf{minimalità}.\medskip
\newline
...
\begin{figure}[H]
    \centering
    \includegraphics[angle=0,width=0.85\textwidth]{../resources/ER/HotelDB_ER_TipologiaCamera.jpeg}
    \caption{Schema E/R per motrare associazione tra CAMERA e TIPOLOGIA\_CAMERA}
\end{figure}
...
\begin{table}[H]
    ...
    \rowcolors{2}{gray!10}{white}
    \small
    \begin{tabular}{|p{5.7cm}|l|c|c|p{5.2cm}|}
        \hline
        \rowcolor{headerblue}
        ...
        \hyperref[sec:CalcolaSconto]{Calcola sconto loyalty per prenotazione}                & Sistema         & I             & 23/g                    & Una volta per prenotazione creata                  \\
...
relativa tavola degli accessi. A differenza di un approccio che selezioni
a priori un sottoinsieme di operazioni "importanti", si è scelto di
analizzare l'intero insieme e di rimandare l'applicazione della regola
80-20 alla sezione di sintesi finale (\cref{sec:sintesiCarico}), una volta
disponibile il costo giornaliero di ciascuna operazione. Questo evita di
polarizzare la selezione su stime di importanza soggettive prima di avere
i dati di carico effettivi.

Convenzioni adottate in tutta la sezione:
\begin{itemize}
    \item il costo degli accessi in scrittura è considerato doppio
          rispetto a quello delle letture (come da \cref{sec:accessi} delle
          slide del corso): costo pesato per esecuzione $= L + 2S$;
    ...
    \item quando il numero di istanze coinvolte non deriva da un semplice
          rapporto nella tavola dei volumi (\cref{tab:volumi}), l'assunzione
...
Operazione di sola inserzione: crea una nuova istanza di \textbf{PERSONA}
e la corrispondente istanza di \textbf{CLIENTE} lungo la generalizzazione
parziale sovrapposta descritta in Progettazione Concettuale. Non coinvolge
alcuna associazione: è l'operazione più semplice della tavola, un buon
punto di riferimento per il limite inferiore di costo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{../resources/nav/HotelDB_Nav_RegistraCliente.jpeg}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|}
        ...
        \hline
        \rowcolor{gray!25}
        \textbf{Totale}   &                    & \textbf{2}       & \textbf{2S}   \\
        \hline
    \end{tabular}
    \caption{Tavola degli accessi --- Registra nuovo cliente}
\end{table}

Costo giornaliero dell'operazione $= (0L + 2S{\times}2) \times 8 = 32$
accessi/giorno.
...
esistente e, come confermato in fase concettuale, inserisce
...
(per recuperare l'identificativo dello stato iniziale) pur non
modificandolo. La lettura di \textbf{APPARTENENZA}/\textbf{LIVELLO\_LOYALTY}
serve a determinare lo sconto loyalty da applicare — la stessa lettura
analizzata isolatamente in \cref{sec:CalcolaSconto}, qui contata come
parte del flusso di creazione.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|}
        ...
        \hline
        \rowcolor{gray!25}
        \textbf{Totale}     &                    & \textbf{6}       & \textbf{4L + 2S} \\
        \hline
    \end{tabular}
    \caption{Tavola degli accessi --- Crea prenotazione}
\end{table}

Costo giornaliero dell'operazione $= (4L + 2S{\times}2) \times 23 = 184$
accessi/giorno.
...
$\text{ASSEGNAZIONE}/\text{PRENOTAZIONE} = 35.000/25.000 = 1{,}4$.
...
di stato (es. Confirmed, CheckedIn). Coerentemente con la decisione presa
in fase concettuale (nessun attributo \textit{StatoAttuale} su
\textbf{PRENOTAZIONE}), l'operazione non deve leggere lo storico
...
Costo giornaliero dell'operazione $= (2L + 1S{\times}2) \times 183 = 732$
accessi/giorno.
...
operazioni che devono \emph{ricostruire} il livello corrente
(\cref{sec:CalcolaSconto}), non qui.
...
\subsection{Calcola sconto loyalty per prenotazione}
\label{sec:CalcolaSconto}

Operazione automatica del Sistema, eseguita una volta per ogni
\textit{Crea prenotazione} (stessa frequenza, 23/g): dato un cliente,
risale tramite \textbf{APPARTENENZA} al \textbf{LIVELLO\_LOYALTY}
correntemente posseduto per leggerne \texttt{ScontoPercentuale}. In
assenza dell'attributo \texttt{DataFine} (decisione concettuale
confermata), individuare la riga ``corrente'' di \textbf{APPARTENENZA}
richiede una interrogazione sullo storico ordinato per
\texttt{DataInizio} (es. \texttt{ORDER BY DataInizio DESC LIMIT 1}, o
\texttt{LEAD()} se serve anche la data di fine derivata). Si stima un
numero medio di righe coinvolte pari al rapporto
$\text{APPARTENENZA}/\text{CLIENTE} = 19.000/14.500 \approx 1{,}3$.

Questa è \textbf{l'operazione al centro del punto aperto di
    Progettazione Logica} già segnalato nella relazione (denormalizzare
\texttt{ScontoPercentuale} su \textbf{CLIENTE} oppure no): la tavola qui
sotto è deliberatamente la versione \emph{senza ridondanza} (schema
concettuale attuale), che servirà come termine di paragone nell'Analisi
delle ridondanze (\cref{sec:analisi-ridondanze} --- da compilare), dove
verrà confrontata con la versione con \texttt{ScontoPercentuale}
duplicato su \textbf{CLIENTE} (1 sola lettura, ma un aggiornamento in più
su ogni \textit{Assegna livello loyalty}, \cref{sec:AssegnaLivello}).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{../resources/nav/HotelDB_Nav_CalcolaScontoLoyalty.jpeg}
    \caption{Schema di navigazione --- Calcola sconto loyalty per prenotazione (senza ridondanza)}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|}
        ...
        \hline
        \rowcolor{gray!25}
        \textbf{Totale}   &                    & \textbf{\textasciitilde{}3,3} & \textbf{tutti L} \\
        \hline
    \end{tabular}
    \caption{Tavola degli accessi --- Calcola sconto loyalty (senza ridondanza)}
\end{table}

Costo giornaliero dell'operazione $\approx 3{,}3 \times 23 \approx 76$
accessi/giorno (baseline senza ridondanza).
...
\textit{Registra consumazione servizio}: stessa osservazione, frequenza
...
temporale reale delle prenotazioni). Come si vedrà in
...
sull'\textbf{sovrapposizione temporale} tra
...
\textbf{TIPOLOGIA\_CAMERA} (per \texttt{PrezzoBase}), applicando lo
sconto loyalty corrente (stesso cammino di \cref{sec:CalcolaSconto}, qui
in versione batch). Si stima il numero di check-out giornalieri pari al
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{6.2cm}|c|c|}
        ...
        \hline
        Consulta disponibilità camere           & 6.300                           & 70,5\%               \\
        Registra consumazione servizio          & 732                             & 78,7\%               \\
        Consulta prenotazioni per data          & 470                             & 84,0\%               \\
        Calcola fatturato giornaliero           & 401                             & 88,5\%               \\
        Cambia stato prenotazione               & 368                             & 92,6\%               \\
        Crea prenotazione                       & 184                             & 94,7\%               \\
        Assegna camera/e a prenotazione         & 166                             & 96,6\%               \\
        Aggiorna stato camera                   & 138                             & 98,2\%               \\
        Calcola sconto loyalty per prenotazione & 76                              & 99,0\%               \\
        Assegna livello loyalty a cliente       & 68                              & 99,7\%               \\
        Registra nuovo cliente                  & 32                              & 100,0\%              \\
        Assegna ruolo a utente                  & 0,3                             & 100,0\%              \\
        Crea/modifica tipologia camera          & 0,1                             & 100,0\%              \\
        Crea/modifica servizio accessorio       & 0,1                             & 100,0\%              \\
        \hline
        \rowcolor{gray!25}
        \textbf{Totale sistema}                 & \textbf{\textasciitilde{}8.936} &                      \\
        \hline
    \end{tabular}
...
\textit{Registra consumazione servizio}, \textit{Consulta prenotazioni
    per data}) coprono da sole l'84\% del carico giornaliero complessivo del
...
correttamente: da sola pesa per il 70,5\% del carico totale, un ordine di
grandezza sopra qualunque altra operazione.

Le operazioni analizzate in dettaglio nel primo tentativo di questa fase
(\textit{Crea prenotazione}, \textit{Assegna camera/e},
\textit{Cambia stato prenotazione}) restano comunque rilevanti per
frequenza assoluta, ma con i dati completi non rientrano nel nucleo
dell'80\% del carico: la loro priorità per l'ottimizzazione (indici,
denormalizzazioni) è quindi secondaria rispetto alle tre sopra indicate.
Questo verrà tenuto presente nel Raffinamento dello schema e
nell'Analisi delle ridondanze (in particolare per la scelta su
\texttt{ScontoPercentuale}, che coinvolge \textit{Calcola sconto loyalty}
--- 99,0\% cumulato, quindi non prioritaria di per sé, ma comunque parte
del cammino di \textit{Crea prenotazione} e \textit{Calcola fatturato
    giornaliero}, entrambe nel nucleo critico).
