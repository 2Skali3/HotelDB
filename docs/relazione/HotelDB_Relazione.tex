...
registrazione --- lo stesso pattern già adottato per l'inserimento
automatico della riga ``Requested'' in \textbf{VARIAZIONE} alla creazione
di una \textbf{PRENOTAZIONE} (\cref{sec:CreaPrenotazione}). Questo
automatismo è stato individuato durante la costruzione degli schemi di
navigazione, come inconsistenza tra lo schema concettuale chiuso e la
tavola degli accessi originariamente redatta; è documentato anche in
Progettazione Concettuale (si veda la nota aggiunta alla sottosezione
``Storico dei Livelli Loyalty'').

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{../resources/nav/HotelDB_Nav_RegistraCliente.jpeg}
...
${APPARTENENZA}/{CLIENTE} = 19.000/14.500 \approx 1{,}3$.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{6.2cm}|c|c|}
        ...
        \hline
        \rowcolor{gray!25}
        \textbf{Totale sistema}                 & \textbf{\textasciitilde{}8.960} &                      \\
        \hline
    \end{tabular}
...
\textit{Registra consumazione servizio}, \textit{Consulta prenotazioni
    per data}) coprono da sole l'83,7\% del carico giornaliero complessivo del
