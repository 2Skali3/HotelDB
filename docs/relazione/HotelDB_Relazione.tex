...
\begin{table}[H]
    ...
    \begin{tabular}{|p{5.7cm}|l|c|c|p{5.2cm}|}
        ...
        \hyperref[sec:ConsultaDisponibilità]{Consulta disponibilità camere in un intervallo} & Reception       & I             & 50/g           & Consultata più volte per prenotazione              \\
...
In questa sezione si proseguirà a mostrare lo schema di navigazione delle operazioni più importanti, e le relative tabelle degli accessi. Nel calcolo degli accessi si stima come doppio il peso degli accessi di scrittura rispetto a quelli di lettura.

\subsection{Crea prenotazione (manca commento)}
...
Costo totale delle operazioni = (4L + 2S * 2) * 23 = 184 al gg
...
L'operazione collega CLIENTE a SERVIZIO passando per CONSUMO. Trattandosi
di un'associazione binaria semplice (CONSUMO è legato solo a CLIENTE, mai
a PRENOTAZIONE, come da decisione di progettazione concettuale), il
cammino logico è lineare e non richiede attraversamenti multipli.
Nonostante sia l'operazione più frequente del sistema (183 occorrenze al
giorno), il suo costo in accessi resta contenuto: è un buon esempio di
come alta frequenza non implichi necessariamente alto carico.
...
\label{sec:ConsultaDisponibilità}

\subsection*{Analisi: criteri di ricerca disponibilità}

La ricerca di camere disponibili può avvenire secondo due criteri, non
alternativi:
\begin{itemize}
    \item per \textbf{tipologia specifica} (es. ``una Deluxe per 2
          persone''): il filtro agisce su \texttt{NomeTipologia};
    \item per \textbf{capacità} (es. ``una camera per 2 persone''): il
          filtro agisce solo sull'attributo \texttt{NumeroLetti} di
          \textbf{TIPOLOGIA\_CAMERA}, indipendentemente dalla tipologia
          specifica.
\end{itemize}
Entrambi i casi sono coperti dagli attributi già presenti in
TIPOLOGIA\_CAMERA, senza necessità di modifiche allo schema concettuale.
Ai fini della tavola degli accessi si considera il caso più oneroso, ossia
la ricerca per capacità, che nel caso peggiore coinvolge camere di più
tipologie (fino alle 80 camere totali), anziché il sottoinsieme di una
...
\texttt{dataCheckIn} e \texttt{dataCheckOut} di ciascuna. È l'unica
operazione tra quelle analizzate il cui costo non deriva direttamente da
un rapporto fisso nella tavola dei volumi, ma da un'assunzione sulla
distribuzione temporale delle prenotazioni: si stima che, per un dato
intervallo di ricerca, siano coinvolte mediamente 15-30 righe di
ASSEGNAZIONE/PRENOTAZIONE su un totale storico di 35.000/25.000. Questo
la rende un'ottima candidata a beneficiare di un indice composito su
